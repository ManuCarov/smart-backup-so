...
void run_benchmark() {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        sys_smart_copy(tests[i].filename, "Resultados/bench_out_sys.bin", SCOPY_OVERWRITE, 0, NULL);
...
void print_help(const char *prog_name) {
    ...
    printf("  %s -b -c -e -a 2 origen destino_seguro.bin\n", prog_name);
...
int main(int argc, char *argv[]) {
    ...
    int option_index = 0;
    while ((opt = getopt_long(argc, argv, "hbpcrea:g:t:C", long_options, &option_index)) != -1) {
        switch (opt) {
